...
In addition, the CAPEXIT instruction can only be used when the `worldswitch` field
...
`worldswitch` field content will cause an exception.
The behaviours of those 
instructions roughly correspond to the CALL and RETURN instructions
respectively.
The differences:

* The CAPENTER instruction sets the `worldswitch` field of the sealed-return capability
...
TODO: add relevant checks to CALL and EXIT as well.
