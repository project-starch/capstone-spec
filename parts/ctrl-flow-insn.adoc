...
to `1` (world switch involved), and changes the `cwrld` register to `1` (secure world).
* The CAPENTER instruction stores the program counter (`pc`) and the stack pointer (`sp`)
to `normal_pc` and `normal_sp` respectively, rather than to a sealed-return capability.
* The CAPEXIT restores the program counter (`pc`) and the stack pointer (`sp`) from
`normal_pc` and `normal_sp` respectively, rather than from a sealed-return capability.
* The CAPEXIT instruction re-seals the context in `switch_cap` rather than a sealed-return
capability specified as an operand.
