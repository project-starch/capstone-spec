...
. Load the content at the memory location `[base, base + CLEN)`,
where `base` is the `base` field of the capability in `cra`, to the program counter (`pc`).
...
. Set the `type` field of the capability in `cra` to `4` (sealed), and write the
...
bewteen the secure world and the normal world. 
