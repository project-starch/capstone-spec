...
Set `x[pc]` to `x[rs1]`, and `x[pc].cursor` to `x[rs1].cursor + imm`.
Meanwhile, former `x[pc]` (`x[pc].cursor` is the address of the next instruction) is written to `x[rd]`.
...
Otherwise, set `x[pc]` to `x[rs1]`, and `x[pc].cursor` to `x[rs1].cursor + imm`.
...
. Set the `x[rs1].type` to `4` (sealed), and write the resulting `x[rs1]` to the register `ceh`.
