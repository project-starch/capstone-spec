...
The CJAL and CBNZ instructions allow jumping to a capability,
i.e., setting the program counter to a given capability,
in a conditional or unconditional manner.

.CJAL instruction format
[wavedrom,,svg]
....
{reg: [
...
For CJAL instruction, the program counter (`pc`) is set to the capability in `rs1`.
Meanwhile, the existing capability in `pc`, with its `cursor` field replaced by the address
of the next instruction, is written to the register `rd`.
