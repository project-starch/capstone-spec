...
.*Discussion: where are the _exception codes_ relevant?*
[%collapsible]
====
****
For _{isa_var_pure}_, there is only 1 place where exception codes are relevant,
which is the argument to pass to the _exception handler domain_.

For _{isa_var_hybrid}_, however, there are 3 places where we need to consider:
...
The argument to pass to the _exception handler domain_ will be in the register `ra`/`cra`,
...
| Unhandleable exception | 30 | N/A in _{isa_var_hybrid}_
|===
...
| Unexpected capability type | The instruction itself (or the lowest XLEN bits if it is wider than XLEN)
| Insufficient capability permissions | The instruction itself (or the lowest XLEN bits if it is wider than XLEN)
| Capability out of bound | The instruction itself (or the lowest XLEN bits if it is wider than XLEN)
| Illegal operand value | The instruction itself (or the lowest XLEN bits if it is wider than XLEN)
...
====
****
The `csp` register is designed to hold data that the in-domain exception handler
can utilize.
As the exception handler is in the same domain as the code that caused the exception,
it is not necessary to seal the content of `csp`, or otherwise prevent the excepted code
from accessing it.
****
====
...
. Store the current `ceh` to the memory location
`[switch_cap.base + CLENBYTES, switch_cap.base + 2 * CLENBYTES)`.
. Store the current `csp` to the memory location
`[switch_cap.base + 2 * CLENBYTES, switch_cap.base + 3 * CLENBYTES)`.
...
. Load new `csp` from memory location `[ceh.base + 2 * CLENBYTES, ceh.base + 3 * CLENBYTES)`.
...
. Store the original `csp` to the memory location `[ceh.base + 2 * CLENBYTES, ceh.base + 3 * CLENBYTES)`.
. For `i = 1, 2, ..., 31`, store the _original_ content of `x[i]` to memory location
`[ceh.base + (i + 2) * CLENBYTES, ceh.base + (i + 3) * CLENBYTES)`.
. Set the `ceh.type` to `5` (sealed-return), `x[rs1].cursor` to `x[rs1].base`, and `ceh.async` to `1` (upon exception).
...
. Store `csp` to the memory location `[switch_cap.base + 2 * CLENBYTES, switch_cap.base + 3 * CLENBYTES)`.
