...
. Swap `ceh` with the content at memory location `[cra.base + CLENBYTES, cra.base + 2 * CLENBYTES)`.
. Write the exception code to the register `a0`.
====
...
.*Note: overview of interrupt handling in _{isa_var_hybrid}_*
[%collapsible]
====
****
...
.*Note: overview of exception handling in _{isa_var_hybrid}_*
[%collapsible]
====
****
...
. For `i = 1, 2, ..., 31`, Swap `x[i]` with the content at 
