...
. Load `csp` from memory location `[cih.base + 2 * CLENBYTES, cih.base + 3 * CLENBYTES]`.
...
. Store the original `csp` to the memory location
...
. Set `cih.type` to `5` (sealed-return), `x[rs1].cursor` to `x[rs1].base`, `cih.reg` to `0`, and `cih.async` to `2` (upon interrupt).
...
. Set the original `ceh.type` to `5` (sealed-return), `x[rs1].cursor` to `x[rs1].base`, `ceh.reg` to `0`, and `ceh.async` to `1` (upon exception).
