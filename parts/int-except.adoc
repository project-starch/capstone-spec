...
. Set `cih.type` to `5` (sealed-return), `cih.reg` to `0`, and `cih.async` to `2` (upon interrupt).
...
. Set the original `ceh.type` to `5` (sealed-return), `ceh.reg` to `0`, and `ceh.async` to `1` (upon exception).
