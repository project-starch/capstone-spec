...
according to the link:#lcc-multiplex[LCC multiplexing table].
====
...
. If `x[rd].type` is `3` (uninitialised) and `x[rd].cursor < x[rs1]`,
set `x[rd].cursor` to `x[rs1]`.
====
...
====
. Write `x[rs1]` to `x[rd]`.
. Set `x[rs1].end` to `x[rs2]`.
. Set `x[rd].base` to `x[rs2]`.
====
...
====
. If `rs2 > 7`, set `x[rs1].perms` to `0`.
Otherwise, set `x[rs1].perms` to `rs2`.
. `MOVC rd, rs1`.
====
...
- `x[rs1].base` is not aligned to `CLEN` bits.
****
...
and `c` link:#alias-definition[aliases] with `x[rs1]`.
* `c.type` is `2` (revocation), `c.valid` is `1` (valid), and `x[rs1] link:#smaller-t-definition[<t] c`.

. `x[rs1].type` is set to `0` (linear)
if, for any invalidated capability `c`, at least one of the following
conditions are met:
