...
* The capability in `rd` has `type = 3` (uninitialised), `type = 4` (sealed), or `type = 5` 
(sealed-return).

*For LCC, an exception is raised if any of the following conditions are met:*

* `rs1` does not contain a capability.
* The capability in `rs1` has `type = 4` (sealed) or `type = 5` (sealed-return).
...
then the `type` field of `rs1` is set to `3` (uninitialised), and its `cursor` field is set
to `base`.
Otherwise, the `type` field of `rs1` is set to `0` (linear).
