...
* _Pure Capstone_
** An exception is raised if any of its operands contains a capability.
...
** An exception is raised if any of its operands contains a capability.
** An exception is raised if the address to be accessed is within the range `[SBASE, SEND)`
(i.e. `addr = x[rs1] + sext(imm)` and `SBASE \<= addr < SEND`).
