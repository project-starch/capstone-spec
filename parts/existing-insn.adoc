...
====
****
In {proj_name}, when using these instructions to access the memory location that does not contain an integer-type value,
the results of the following operations are _undefined_:

* Load an integer from the memory location.
* Store an integer to the memory location and then load an integer from the rest of the `CLENBYTES`-byte aligned memory location.
****
====
...
link:#additional-regs[one defined in {isa_name}],
or when the link:#csr-man-constr[read/write constraints] are not satisfied.
* `ecall`.
* `ebreak`.
****
