...
|===
|{cheri_isa_name} instruction(s) |{isa_name} instruction(s) | {cheriot_isa_name} instruction(s)
...
|LDC  | LC.CAP, CLC | CLC
|STC  | SC.CAP, CSC | CSC
|LD[BHWD] | L[BHWD][U].CAP | L[BHWD][U]
|ST[BHWD] | S[BHWD][U].CAP | S[BHWD][U]
|LDCR | LC.DDC, CLC | -
|STCR | SC.DDC, CSC | -
...
with {isa_name} (more specifically, {isa_var_hybrid})
