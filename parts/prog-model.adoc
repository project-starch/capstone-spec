...
|`cih` | - | The original content must be an invalid capability (`valid = 0`)
|`cinit` | One-time only | -
|===
...
During system initialisation, the content of `cinit` is set by the hardware according to the
following rules:

.Initial Capability in `cinit`
[%header%autowidth.stretch]
|===
...
capability in `cinit` and store it in a general-purpose register.
This operation can only be performed once.
Any attempt to write `cinit` or read it for the second time will be silently ignored.
****
