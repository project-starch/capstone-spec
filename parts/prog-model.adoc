...
* link:#additional-regs[`cwrld`] is `1` (secure world) and any of the conditions for _{isa_var_pure}_ are met.
* link:#additional-regs[`cwrld`] is `0` (normal world) and any of the conditions for {base_isa_name} are met.
* Instruction access fault (1)
- link:#additional-regs[`cwrld`] is `0` (normal world) and `pc` does not contain an integer.
...
* _{isa_var_hybrid}_:
** link:#additional-regs[`cwrld`] is `1` (secure world): Same as _{isa_var_pure}_.
** link:#additional-regs[`cwrld`] is `0` (normal world): The instruction pointed to by `pc` is fetched and executed.
...
_capability control and status registers (CCSRs)_. Under their respective constraints,
CCSRs can be manipulated using link:#ctrl-status[CCSR manipulation instructions].
...
.Note
****
`cinit` is a special CCSR that is used to
bootstrap capabilities after a link:#reset[system reset].
link:#ctrl-status[CCSR manipulation instructions] can be used to read this initial
capability and store it in a general-purpose register. This operation can only
be performed once after each reset.
...
* `ceh`, `deh`, `cih`, and `epc` contain either integers or
...
* Each general-purpose register either contains an integer, or a capability with `valid = 0`.
* No addressable memory location can contain a capability.
* `ceh` and `deh` contain either integers or
capabilities with `valid = 0` (invalid).
