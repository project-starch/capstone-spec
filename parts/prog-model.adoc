...
In _Pure Capstone_, the memory can only be accessed through capabilities.
...
In _TransCapstone_, the physical memory is divided into two disjoint regions:
...
| _Normal Memory_ | `[0, SBASE) U [SEND, 2^XLEN)` | MMU | link:#riscv-load-store[RISC-V load/store instructions],
...
| _Pure Capstone_ | `1` (linear) | `0` | `0` | `2^XLEN` | `4` (read-write-execute) | `1` (valid)
