...
* link:#ccsrs[`cwrld`] is `1` (secure world) and any of the conditions for _{isa_var_pure}_ are met.
* link:#ccsrs[`cwrld`] is `0` (normal world) and any of the conditions for {base_isa_name} are met.
* Instruction access fault (1)
** link:#ccsrs[`cwrld`] is `0` (normal world) and `pc` does not contain an integer.
...
* _{isa_var_hybrid}_:
** link:#ccsrs[`cwrld`] is `1` (secure world): Same as _{isa_var_pure}_.
** link:#ccsrs[`cwrld`] is `0` (normal world): The instruction pointed to by `pc` is fetched and executed.
...
During system initialisation, `cinit` is set by the hardware
...
capability of `cinit` and store it in one of the general-purpose registers.
`cinit` is set to `cnull` after the first operation and any attempt to
write to `cinit` is silently ignored.
****
