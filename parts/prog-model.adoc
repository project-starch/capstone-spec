...
|`cursor` | `0..2^XLEN-1` |  Only applicable when `type = 0` (linear), `type = 1` (non-linear), or `type = 3` (uninitialised).
...
|`perms` | `0..7` | Only applicable when `type = 0` (linear), `type = 1` (non-linear), or `type = 3` (uninitialised).
...
|`async` | `0..1` | Only applicable when `type = 4` (sealed) or `type = 5` (sealed-return).
Whether the capability is sealed asynchronously: `0` = synchronously, `1` = asynchronously
...
* _{isa_var_pure}_: The program counter (`pc`) is extended to contain a capability only.
...
** `2 \<=p x[pc].perms` does not hold.
** `x[pc].cursor` is not in the range `[pc.base, pc.end - 4]`.
* _{isa_var_hybrid}_
** link:#ccsrs[`cwld`] is `1` (secure world) and any of the conditions for _{isa_var_pure}_ are met.
** link:#ccsrs[`cwld`] is `0` (normal world) and `x[pc]` is not an integer.
** link:#ccsrs[`cwld`] is `0` (normal world) and any of the conditions for {base_isa_name} are met.

*If no exception is raised:*

* _{isa_var_pure}_: The instruction pointed to by `x[pc].cursor` is fetched and executed.
The `x[pc].cursor` is then incremented by `4` (i.e., `x[pc].cursor += 4`).
* _{isa_var_hybrid}_:
** link:#ccsrs[`cwld`] is `1` (secure world): Same as _{isa_var_pure}_.
** link:#ccsrs[`cwld`] is `0` (normal world): The instruction pointed to by `x[pc]` is fetched and executed.
The `x[pc]` is then incremented by `4` (i.e., `x[pc] += 4`).
...
!===
! Mnemonic ! CCSR encoding ! Description
...
This register needs to be stored during context switching in normal world
...
|`cinit` | No constraint | Silently ignored
|===
...
.Note
****
`cinit` is a capability CSR used to provide the initial capability in the system.
During system initialisation, `x[cinit]` is set by the hardware
to a linear capability as follows:

.Initial Capability of `x[cinit]`
[%header%autowidth.stretch]
|===
| Variant | `type` | `cursor` | `base` | `end` | `perms` | `valid`
| _{isa_var_pure}_ | `1` (linear) | `0` | `0` | `2^XLEN` | `7` (read-write-execute) | `1` (valid)
| _{isa_var_hybrid}_ | `1` (linear) | `SBASE` | `SBASE` | `SEND` | `7` (read-write-execute) | `1` (valid)
|===

link:#ccsr-man[CCSR manipulation instructions] can be used to read the initial
capability of `x[cinit]` and store it in one of the general-purpose registers.
`x[cinit]` is set to `x[cnull]` after the first operation and any attempt to
write to `x[cinit]` is silently ignored.
****
