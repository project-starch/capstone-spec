...
* Instruction access fault (1)
** link:#ccsrs[`cwld`] is `0` (normal world) and `pc` does not contain an integer.

*If no exception is raised:*

* _{isa_var_pure}_: The instruction pointed to by `pc.cursor` is fetched and executed.
The `pc.cursor` is then incremented by `4` (i.e., `pc.cursor += 4`).
* _{isa_var_hybrid}_:
** link:#ccsrs[`cwld`] is `1` (secure world): Same as _{isa_var_pure}_.
** link:#ccsrs[`cwld`] is `0` (normal world): The instruction pointed to by `pc` is fetched and executed.
The `pc` is then incremented by `4` (i.e., `pc += 4`).
