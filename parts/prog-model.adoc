...
====
****
`cwld` is a special register added in _{isa_var_hybrid}_ that indicates the currently executed world.
Please see link:#additional-regs[Added Registers] for details.
****
====
...
====
_{isa_var_pure}_ or _{isa_var_hybrid}_ secure world (i.e., `cwld = 1`)
...
_{isa_var_hybrid}_ normal world (i.e., `cwld = 0`)
...
! `epc` ! - ! - ! The exception program counter register
...
! `switch_cap`! `0x003` !-! The capability used to store contexts when switching worlds asynchronously
! `exit_reg`!-!-! The index of the general-purpose register for receiving the exit code when exiting the secure world 
! `tval` ! - ! `0x801` ! The exception data (trap value) register
! `cause` ! - ! `0x802` ! The exception cause register
! `emode` ! - ! `0x803` ! The encoding mode of the machine. `0` = integer encoding mode, `1` = capability encoding mode
!===
|===
...
CCSRs can be manipulated using _link:#ctrl-status[control and status instructions]_.
...
link:#ctrl-status[control and status instructions] can be used to read the initial
