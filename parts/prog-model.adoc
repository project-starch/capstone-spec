...
|`cursor` | `0..2^XLEN-1` |  Not applicable when `type = 4` (sealed) or `type = 5` (sealed-return).
...
|`perms` | `0..4` | Not applicable when `type = 4` (sealed) or `type = 5` (sealed-return).
...
|`worldswitched` | `0..1` | Only applicable when `type = 5` (sealed-return). Whether a world
switch is involved. `0` = no world switch, `1` = world switch
|===
...
Similar to the general-purpose registers, the program counter (`pc`)
is also extended to contain a capability or an integer.
