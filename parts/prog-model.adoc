...
or equivalently, `XLENBYTES = 8` bytes.
...
{proj_name} currently supports two variants of the ISA, i.e., _{isa_var_pure}_ and _{isa_var_hybrid}_.
While _{isa_var_pure}_ is a pure capability-based ISA, _{isa_var_hybrid}_ is a hybrid ISA that
supports both capabilities and traditional virtual memory. In _{isa_var_hybrid}_, the memory
...
* `x[pc].cursor` is not in the range
`[pc.base, pc.end-4]` (i.e., `pc.cursor < pc.base` or `pc.cursor > pc.end-4`).
...
The {isa_name} instruction set is based on the RV64G instruction set.
