...
supports both capabilities and traditional virtual memory. In _TransCapstone_, the memory
...
! `cih`  ! The sealed capability for the interrupt handler
!===
...
TODO: talk about how to read/write those registers, especially `ceh` and `cih`, which are crucial
